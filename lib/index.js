...
class CacheBuilder {
        ...
        try {
            ...
            if (authResult === OK_RESPONSE) {
                ...
                redisClientVar.on("end", async () => {
                    this._enableLogging && this._logger.error(`😥😥😥 ${name} ${host}:${port} Disconnected`);
                });
    ...
    async _buildCacheByFn(cacheName, dataFetchingFn, isGenerator, cacheKeys, redisDataTypes, idProp) {
        if (!this.isMaster) {
        ...
        try {
            let dataFetchingFnExecuted = await dataFetchingFn();
            ...
            for (let i = 0; i < cacheKeys.length; i++) {
                ...
                if (dataType === 'string') {
                    let stringifiedData = JSON.stringify(dataFetchingFnExecuted);
                    ...
                    this._pubClient.hset(temporaryKey, stringifiedData);
                    await this._pubClient.rename(temporaryKey, cacheKey);
                } else {
                    if (isGenerator === true) {
                        let currentItem = await dataFetchingFnExecuted.next();
                        while (!currentItem.done) {
                            let currentValue    = currentItem.value;
                            let stringifiedData = JSON.stringify(currentValue);
                            if (dataType === 'list') {
                                let listLength = await this._pubClient.lpush(temporaryKey, stringifiedData);
                            } else if (dataType === 'hash') {
                                this._pubClient.hset(cacheKey, currentValue[idProp], stringifiedData);
                            }
                        }
                    } else {
                        if (dataType === 'list') {
                            if (!_.isArrayLikeObject(dataFetchingFnExecuted)) {
                                this._logger.error(`buildCacheByFn ${cacheName}: function should return an array, cache is not refreshed!`);
                                return;
                            }

                            let reverseData     = _.reverse(dataFetchingFnExecuted);
     ...
     *    - `dataFetchingFn`: function to fetch data. 'list' data type expects an array, 'hash'  data type expects an array of object, could be a generator.
     *    - `isGenerator`: whether dataFetchingFn is generator.
    ...
    async register(options) {
        ...
        let {dataFetchingFn, cacheKeys, redisDataTypes, refreshInterval, idProp, name, isGenerator} = options;

        if (!typeof dataFetchingFn === 'function') {
        ...
        if (isGenerator === true && _.indexOf('string') >= 0) {
            throw Error('string cache type can not be used together with generator data fetching function')
        }
        ...
        await this._buildCacheByFn(name, dataFetchingFn, isGenerator, cacheKeys, redisDataTypes, idProp);

        let intervalId = setInterval(function () {
            self._buildCacheByFn(name, dataFetchingFn, isGenerator, cacheKeys, redisDataTypes, idProp);
